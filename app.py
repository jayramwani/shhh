...
#         logger.info(f"OTP verified successfully for {email}. Generating PIN.")
#         pin = secrets.token_hex(4)  # Generate a secure random PIN
#         pin_storage[email] = pin  # Store PIN in memory with expiration
#         logger.info(f"Generated PIN for {email}: {pin}")
...
#         if time.time() - pin_storage[email]['timestamp'] < PIN_EXPIRATION_TIME:
#             return jsonify({"pin": pin})
#         else:
#             del pin_storage[email]  # Remove expired PIN
#             logger.warning(f"PIN for {email} has expired.")
#             return jsonify({"error": "PIN has expired"}), 403
#     else:
#         logger.warning(f"No PIN found for {email}.")
